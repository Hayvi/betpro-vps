...
const Input = forwardRef(
  ({ className, type, value, onChange, showPaste = false, showSearch = false, variant = 'default', error, ...props }, ref) => {
    ...
    return (
      <div className="relative w-full">
        <input
          ...
          ref={combinedRef}
          className={cn(
            "block w-full rounded-lg text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50",
            variant === 'dashboard'
              ? "dash-control"
              : "w-full px-5 py-3 rounded-xl border ui-trans-fast bg-slate-900/40 border-slate-800/80 text-white placeholder:text-slate-500 focus:outline-none focus:bg-slate-900/60 focus:border-violet-500/50 focus:ring-4 focus:ring-violet-500/10",
